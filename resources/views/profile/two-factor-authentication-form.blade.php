<x-action-section>
    ...
    <x-slot name="description">
        {{ __('Ajoutez une sécurité supplémentaire à votre compte en activant l\'authentification à deux facteurs') }}
    </x-slot>
    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Finalisez l\'activation de l\'authentification à deux facteurs.') }}
                @else
        ...
        <div class="mt-3 max-w-xl text-sm text-gray-600 dark:text-gray-400">
            <p>
                {{ __('Lorsque l\'authentification à deux facteurs est activée, vous serez invité à fournir un jeton sécurisé et aléatoire pendant l\'authentification. Vous pouvez récupérer ce jeton depuis l\'application Google Authenticator sur votre téléphone.') }}
            </p>
        </div>
            ...
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600 dark:text-gray-400">
                    <p class="font-semibold">
                        @if ($showingConfirmation)
                            {{ __('Pour finaliser l\'activation de l\'authentification à deux facteurs, scannez le code QR suivant à l\'aide de l\'application d\'authentification de votre téléphone, ou saisissez la clé de configuration et fournissez le code OTP généré.') }}
                        @else
                ...
                @if ($showingRecoveryCodes)
                    <x-confirms-password wire:then="regenerateRecoveryCodes">
                        <x-secondary-button class="mr-3">
                            {{ __('Générez de nouveaux codes de récupération') }}
                        </x-secondary-button>
                    </x-confirms-password>
